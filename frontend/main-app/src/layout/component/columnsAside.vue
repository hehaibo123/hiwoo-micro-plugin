		...
		<el-scrollbar>
			<ul @mouseleave="onColumnsAsideMenuMouseleave()">
				<li
					...
					:title="$t(v.title)"
				>
					<div :class="themeConfig.columnsAsideLayout" v-if="!v.isLink || (v.meta.isLink && v.meta.isIframe)">
						<SvgIcon :name="v.icon" />
						<div class="columns-vertical-title font12">
							{{
								$t(v.title) && $t(v.title).length >= 4
									? $t(v.title).substr(0, themeConfig.columnsAsideLayout === 'columns-vertical' ? 4 : 3)
									: $t(v.title)
							}}
						</div>
					</div>
