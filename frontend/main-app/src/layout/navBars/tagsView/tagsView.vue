    ...
    <el-scrollbar ref="scrollbarRef" @wheel.prevent="onHandleScroll">
      <ul class="layout-navbars-tagsview-ul" :class="setTagsStyle" ref="tagsUlRef">
        ...
        >
          ...
          <SvgIcon :name="v.icon" v-if="!isActive(v) && getThemeConfig.isTagsviewIcon" class="pr5"/>
...
const isActive = (v: RouteItem) => {
  if (getThemeConfig.value.isShareTagsView) {
    ...
    if ((v.query && Object.keys(v.query).length) || (v.params && Object.keys(v.params).length)) {
      ...
      return (v.activeRule + v.path) === state.routePath;
    }
  }
};
...
const getTagsViewRoutes = async () => {
  ...
  state.routePath = (await route.isDynamic) ? route.isDynamicPath : route.path;
  state.tagsViewList = [];
...
const initTagsView = async () => {
  if (Session.get('tagsViewList') && getThemeConfig.value.isCacheTagsView) {
    ...
    await state.tagsViewRoutesList.map((v: RouteItem) => {
      // if (v.meta?.isAffix && !v.isHide) {
      if (v.meta?.affix && !v.hide) {
...
const solveAddTagsView = async (path: string, to?: RouteToFrom) => {
  let isDynamicPath = to?.meta?.isDynamic ? to.isDynamicPath : path;
  let current = state.tagsViewList.filter(
      (v: RouteItem) =>
          (v.activeRule + v.path) === isDynamicPath &&
          isObjectValueEqual(
  ...
  if (current.length <= 0) {
    ...
    let findItem = state.tagsViewRoutesList.find((v: RouteItem) => (v.activeRule + v.path) === isDynamicPath);
...
const addTagsView = (path: string, to?: RouteToFrom) => {
  ...
  nextTick(async () => {
    ...
    if (to?.meta?.isDynamic) {
      ...
      if (state.tagsViewList.some((v: RouteItem) => (v.activeRule + v.path) === path)) {
        // 防止首次进入界面时(登录进入) tagsViewList 不存浏览器中
        addBrowserSetSession(state.tagsViewList);
        return false;
      }
      item = state.tagsViewRoutesList.find((v: RouteItem) => (v.activeRule + v.path) === path);
    }
...
const openCurrenFullscreen = async (path: string) => {
  ...
  if (item.isDynamic) await router.push({name: item.name, params: item.params});
...
const onTagsClick = (v: RouteItem, k: number) => {
  ...
  // router.push(v);
  router.push(v.activeRule + v.path);
  // 分栏布局时，收起/展开菜单
...
const setTagsViewHighlight = (v: RouteToFrom) => {
  ...
  return `${v.meta?.isDynamic ? v.isDynamicPath : v.path}-${path}`;
};
// 鼠标滚轮滚动
...
onBeforeRouteUpdate(async (to) => {
  ...
  state.routePath = to.isDynamic ? to.isDynamicPath : to.path;
  ...
  .tags-style-five {
    ...
    .layout-navbars-tagsview-ul-li {
      ...
      &:hover {
        ...
        background: var(--el-is-active-navbar-bg-color);
        color: unset;
      }
    }
    ...
    .is-active {
      @extend .tags-style-five-svg;
      background: var(--el-is-active-navbar-bg-color) !important;
      color: var(--el-is-active-navbar-color) !important;
      z-index: 1;
    }
  }
}
