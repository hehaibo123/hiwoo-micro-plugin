            ...
            if (v.path) {
                ...
                if(v.children){
                    if(v.children.length>0){
                      if(v.children[0].path ===''|| v.children[0].path === null){
                          v.children= null;
                      }
                    }
                ...
                if (newArr[0].isKeepAlive && v.isKeepAlive) {
...
router.beforeEach(async (to, from, next) => {
    ...
    if (to.title) NProgress.start();
    const token = Session.get('satoken');
    if (to.href.includes('secureData')) {
        ...
        next();
        NProgress.done();
    } else if ( to.href.includes('jumpData')) {
        Session.clear();
        Local.clear();
